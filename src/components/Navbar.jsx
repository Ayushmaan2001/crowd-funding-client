    ...
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className='sm:flex hidden flex-row justify-end gap-4'>
        <CustomButton
          ...
          handleClick={() => {
            if (address) naviate('create-campaign')
            else connect()
          }
          }
        />
      ...
      {/* Mobile View */}
      <div className='sm:hidden flex justify-between icons-center relative'>
        ...
        <img
          src={menu}
          alt='menu'
          className='w-[34px] h-[34px] rounded-[10px] object-contain cursor-pointer'
          onClick={() => settoogleDrawer((prev) => !prev)}
        />
        <div
          className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-10 shadow-secondary py-5 ${!toogleDrawer ? '-translate-y-[100vh]' : 'translate-y-0'} transition-all duration-700`}
        >
          <ul className='mb-4 '>
            {navlinks.map((link) => (
              <li
